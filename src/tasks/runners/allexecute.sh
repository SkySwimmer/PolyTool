...
function taskRunnerProjectAllPrepare() {
    ...
    if [ "$onlyWhenNeeded" == "true" ]; then
        ...
        if arrayContains "$taskProjectId-$task" TASKSBEINGRUN_PREPARE && ! arrayContains "$taskProjectId-$task" TASKS_PERMITTING_MULTIRUN; then
            ...
            if arrayContains "RUNTIME@$taskProjectId@$task" TASKS_FOUND || arrayContains "RUNTIME@$task" TASKS_FOUND; then
                taskFound=true
            fi
            return 0
        fi
    ...
    if [ "$task" != "restore" ]; then 
        ...
        for depId in "${dependenciesList[@]}"; do
            ...
            # Find in dependency
            taskRunnerProjectAllPrepare "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                return $exit
            fi
            ...
            if [ "$LOCALPROJECTID" != "$projectId" ]; then
                runLocalToProject "$projectId" taskRunnerProjectAllPrepare "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
                local exit=$?
                if [ "$exit" != 0 ]; then
                    return $exit
                fi
    ...
    if [ -d "$runtimeTaskDir" ]; then
        ...
        if [ "$doRun" == "true" ]; then
            ...
            # Find task
            taskSensitiveRunLocalToProject false "$projectId" "$task" "$projectId" execTasksAllPrepare "$task" "$runtimeTaskDir" false "" "" "${runnerArgs[@]}"
            local exit=$?
    ...
    for subProjectId in "${subprojectsList[@]}"; do
        ...
        # Run in subproject
        taskRunnerProjectAllPrepare "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
        local exit=$?
        if [ "$exit" != 0 ]; then
            # Revert list
            ANTIRECURSIONLIST=("${callTaskListLast[@]}")
        ...
        # Run dependency in local project so the task is run in dependencies
        if [ "$LOCALPROJECTID" != "$projectId" ]; then
            runLocalToProject "$projectId" taskRunnerProjectAllPrepare "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                return $exit
            fi
...
function taskRunnerProjectAllRun() {
    ...
    if [ "$task" != "restore" ]; then 
        ...
        for depId in "${dependenciesList[@]}"; do
            ...
            # Run in dependency
            taskRunnerProjectAllRun "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                # Revert list
                ANTIRECURSIONLIST=("${callTaskListLast[@]}")
            ...
            # Run dependency in local project so the task is run in dependencies
            if [ "$LOCALPROJECTID" != "$projectId" ]; then
                runLocalToProject "$projectId" taskRunnerProjectAllRun "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
                local exit=$?
                if [ "$exit" != 0 ]; then
                    return $exit
                fi
    ...
    if [ -d "$runtimeTaskDir" ]; then
        ...
        if [ "$doRun" == "true" ]; then
            ...
            # Find task
            taskSensitiveRunLocalToProject false "$projectId" "$task" "$projectId" execTasksAllRun "$task" "$runtimeTaskDir" false "" "" "${runnerArgs[@]}"
            local exit=$?
    ...
    for subProjectId in "${subprojectsList[@]}"; do
        ...
        # Run in subproject
        taskRunnerProjectAllRun "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
        local exit=$?
        if [ "$exit" != 0 ]; then
            # Revert list
            ANTIRECURSIONLIST=("${callTaskListLast[@]}")
        ...
        # Run dependency in local project so the task is run in dependencies
        if [ "$LOCALPROJECTID" != "$projectId" ]; then
            runLocalToProject "$projectId" taskRunnerProjectAllRun "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                return $exit
            fi
...
function taskRunnerProjectAllFinish() {
    ...
    if [ "$task" != "restore" ]; then 
        ...
        for depId in "${dependenciesList[@]}"; do
            ...
            # Run in dependency
            taskRunnerProjectAllFinish "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                # Revert list
                ANTIRECURSIONLIST=("${callTaskListLast[@]}")
            ...
            # Run dependency in local project so the task is run in dependencies
            if [ "$LOCALPROJECTID" != "$projectId" ]; then
                runLocalToProject "$projectId" taskRunnerProjectAllFinish "$onlyWhenNeeded" "$task" "$depId" "$depDir" "${runnerArgs[@]}"
                local exit=$?
                if [ "$exit" != 0 ]; then
                    return $exit
                fi
    ...
    if [ -d "$runtimeTaskDir" ]; then
        ...
        if [ "$doRun" == "true" ]; then
            ...
            # Find task
            taskSensitiveRunLocalToProject false "$projectId" "$task" "$projectId" execTasksAllFinish "$task" "$runtimeTaskDir" false "" "" "${runnerArgs[@]}"
            local exit=$?
    ...
    for subProjectId in "${subprojectsList[@]}"; do
        ...
        # Run in subproject
        taskRunnerProjectAllFinish "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
        local exit=$?
        if [ "$exit" != 0 ]; then
            # Revert list
            ANTIRECURSIONLIST=("${callTaskListLast[@]}")
        ...
        # Run dependency in local project so the task is run in dependencies
        if [ "$LOCALPROJECTID" != "$projectId" ]; then
            runLocalToProject "$projectId" taskRunnerProjectAllFinish "$onlyWhenNeeded" "$task" "$subProjectId" "$subProjectDir" "${runnerArgs[@]}"
            local exit=$?
            if [ "$exit" != 0 ]; then
                return $exit
            fi
